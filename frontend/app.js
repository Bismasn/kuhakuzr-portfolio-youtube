      ...
      if (gameActivity) {
        ...
        if (gameDetails)
          gameDetails.textContent =
            gameActivity.details || gameActivity.state || "Sedang dimainkan";
      } else {
        if (gameTitle) gameTitle.textContent = "Tidak Sedang Bermain";
        if (gameDetails)
          gameDetails.textContent = `Status Discord: ${data.discord_status.toUpperCase()}`;
      }
    } else {
      if (gameTitle) gameTitle.textContent = "User Tidak Ditemukan";
      if (gameDetails)
        gameDetails.textContent = "Pastikan sudah join server Discord Lanyard.";
    }
  } catch (error) {
